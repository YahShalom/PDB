...
export async function GET() {
  ...
  for (const column of orderColumns) {
    const result = await supabaseServer.from('services').select('*').order(column, { ascending: true });
  ...
  if (error) {
    return NextResponse.json(
      { error: normalizeSupabaseError(error, 'Supabase services fetch failed').message },
      { status: 500 }
    );
  }
...
export async function POST(req: NextRequest) {
  ...
  let { data, error } = await supabaseServer.from('services').insert(body).select('id').single();

  if (error && isMissingColumn(error)) {
    const mapped = mapServicePayload(body);
    ({ data, error } = await supabaseServer.from('services').insert(mapped).select('id').single());
  }

  if (error) {
    return NextResponse.json(
      { error: normalizeSupabaseError(error, 'Supabase services insert failed').message },
      { status: 500 }
    );
  }
