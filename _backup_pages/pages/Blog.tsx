              ...
              <ScrollReveal key={post.id} delay={`${idx * 0.1}s`}>
                <article className="bg-white dark:bg-charcoal-800 rounded-2xl flex flex-col overflow-hidden group h-full floating-card gold-border-gradient gold-card-text">
                  ...
                  <div className="p-8 flex-grow flex flex-col relative z-10">
                    ...
                    <h2 className="text-2xl font-serif font-medium text-charcoal dark:text-cream mb-4 group-hover:text-gold transition-colors cursor-pointer">
                      {post.title}
                    </h2>
                    <p className="text-charcoal-600 dark:text-cream-200/70 mb-6 leading-relaxed font-light flex-grow">
                      {post.excerpt}
                    </p>
                    <button className="text-charcoal-900 dark:text-white font-bold text-xs uppercase tracking-widest hover:text-gold transition-colors inline-flex items-center mt-auto">
                      {labels.readMore} <ArrowRight size={14} className="ml-2 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </article>
              </ScrollReveal>
